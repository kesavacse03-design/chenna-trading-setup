...
async function runBacktest(params, hooks){
  ...
  // Sanitize symbol list and strategy config
  const SYMS = Array.isArray(symbols) ? symbols.filter(Boolean) : [];
  try {
    if (params && typeof params.strategyConfig === 'object' && params.strategyConfig) {
      const cfg = params.strategyConfig;
      const toNum = (v,d) => { const n = Number(v); return Number.isFinite(n) ? n : d; };
      cfg.targetR = Math.max(0.05, toNum(cfg.targetR, 1.2));
      cfg.atrStop = Math.max(0.05, toNum(cfg.atrStop, 1.0));
      if (cfg.rsiMin !== undefined || cfg.rsiMax !== undefined) {
        cfg.rsiMin = toNum(cfg.rsiMin, 30);
        cfg.rsiMax = toNum(cfg.rsiMax, 70);
        if (cfg.rsiMin > cfg.rsiMax) { const t = cfg.rsiMin; cfg.rsiMin = cfg.rsiMax; cfg.rsiMax = t; }
      }
      params.strategyConfig = cfg;
    }
  } catch(_){}
  ...
  let lastLogIdx = 0;
  // rejection aggregation when quietRejects is enabled
  const quietRejects = Boolean(params && params.quietRejects) || String(process.env.BACKTEST_QUIET_REJECTS || '') === '1';
  const rejectCounters = {}; // { symbol: { count: number, samples: Set<string> } }
  const allTrades = [];
  const skipped = [];
  let processed = 0;
  hooks && hooks.onProgress && hooks.onProgress({ phase:'run', done: 0, total: SYMS.length });
  ...
  if (!fs.existsSync(cachePath) && !fs.existsSync(altCachePath)) {
      try {
        ...
        try {
          const { MarketDataAdapter, loadConfig } = require('../marketdata/index.cjs');
            ...
            // If the MarketDataAdapter returned an empty array in mock mode,
            // try the simple MockDataAdapter as a last-resort so prefetched
            // runs don't skip symbols that can be synthesized locally.
            try {
              const wantMockFallback = ((mode || 'mock') === 'mock');
              if (wantMockFallback && Array.isArray(data) && data.length === 0) {
                const { MockDataAdapter } = require('./dataAdapter.cjs');
                const alt = await new MockDataAdapter().fetch({ symbol: sym, from, to, interval });
                if (Array.isArray(alt) && alt.length) data = alt;
              }
    ...
    // Normalize and validate OHLCV
    try {
      if (Array.isArray(replay.data)) {
        replay.data = replay.data.map(d => {
          if (Array.isArray(d)) { const [ts,o,h,l,c,v] = d; return { date: (ts? new Date(ts).toISOString(): null), open:Number(o), high:Number(h), low:Number(l), close:Number(c), volume:Number(v||0) }; }
          return { date: d.date || d.time || null, open: Number(d.open), high: Number(d.high), low: Number(d.low), close: Number(d.close), volume: Number(d.volume||0) };
    ...
    if (replay.data.length < 20) {
      skipped.push({ symbol: sym, reason: 'insufficient-candles', have: replay.data.length });
      try { fs.writeFileSync(path.join(checkpointsDir, `${sanitizeSym(sym)}.json`), JSON.stringify({ runId, symbol: sym, status: 'done', skipped: true, reason: 'insufficient-candles', have: replay.data.length }, null, 2), 'utf8'); } catch(_){ }
      continue;
    }
    // Optional per-symbol indicator cache priming: if indicator cache exists, attach to strategy instance
    try {
      const ic = require('./indicatorCache.cjs');
      const cached = ic.get(sym, from, to, interval, 'ohlcv', null);
      if (cached && Array.isArray(cached)) {
        // no-op for now; indicator cache is used by strategies/indicators directly where supported
      } else {
        // store raw ohlcv in cache for reuse by other workers in same process
        try { ic.set(sym, from, to, interval, 'ohlcv', null, replay.data.slice()); } catch(_){}
      }
    } catch(_){ }
    if (testForce) {
      try {
  ...
  if ((mode || 'mock') === 'mock') {
  // signal permissive mode to downstream components
  try { process.env.BACKTEST_PERMISSIVE = '1'; process.env.MOCK_MODE = '1'; } catch(_){}
  strat.setConfig({ N: 1, volumeFactor: 0.5, atrStop: 0.4, targetR: 0.8, qty: 100, minRisk: 0.05, permissive: true });
    } else {
      strat.setConfig({ N: 3, volumeFactor: 0.9, atrStop: 1.0, targetR: 1.2, qty: 100 });
    }
  }
    // If the caller explicitly provided a permissive strategyConfig, signal downstream
    // components (simulator, strategy) to operate in permissive/mock mode for quick smoke runs.
    try {
      if (usedCfg && usedCfg.permissive) {
        try { process.env.BACKTEST_PERMISSIVE = '1'; process.env.MOCK_MODE = '1'; } catch(_){}
      }
    } catch(_){}
  // expose context on strategy so strategyBase can look up cache seeds
  try { strat._symbol = sym; strat._from = from; strat._to = to; strat._interval = interval; strat._mock = ((mode||'mock') === 'mock'); } catch(_){ }

  // Precompute and cache EMA series for common periods to speed repeated indicator init
  try {
    const ic = require('./indicatorCache.cjs');
    const inds = require('./indicators.cjs');
    const cachedOhlcv = ic.get(sym, from, to, interval, 'ohlcv', null);
    const ohlcv = Array.isArray(cachedOhlcv) ? cachedOhlcv : replay.data;
    const periods = [8,13,21,50,89];
    for (const p of periods){
      const key = ic.get(sym, from, to, interval, 'EMA', { period: p });
      if (!key) {
        try {
          const series = inds.computeSeries('EMA', ohlcv, { period: p });
          // store last 'period' values as seed buffer
          const seed = Array.isArray(series) ? series.filter(v=>v!==null) : [];
          if (seed.length) ic.set(sym, from, to, interval, 'EMA', { period: p }, seed);
        } catch(_){}
      }
    }
  } catch(_){}
  ...
  replay.start({symbol:sym, from, to}, { interval }, c => {
    strat.onCandle(c);
          ...
          if (L.action === 'REJECTED') {
            ...
            try {
              ...
              for (const f of filters) {
                ...
                // write to rejection file always
                try { fs.appendFileSync(rejectionPath, JSON.stringify({ runId, symbol: sym, filter: f, gates: L.gates||{}, time: L.time||null }) + '\n', 'utf8'); } catch(_){ }
                if (quietRejects) {
                  // aggregate counters and keep few samples
                  const ctr = (rejectCounters[sym] = rejectCounters[sym] || { count: 0, samples: new Set() });
                  ctr.count++;
                  if (ctr.samples.size < 3) ctr.samples.add(msg);
                } else {
      ...
      if (open) {
        ...
        const preferTargetFirst = !!(strat && strat.cfg && strat.cfg.permissive);
        // increment bars held per bar
        open.barsHeld = (open.barsHeld || 0) + 1;
            // In permissive/mock mode, evaluate target hit before stop to avoid pessimistic bias
            if (preferTargetFirst && high >= open.target) {
  ...
  logRun(`EXIT ${open.tradeId} TARGET_HIT barsHeld=${holdingDays}`);
      ...
      if (p && !open){
        try {
          ...
          strat.onFill(fill);
          // After onFill, strategy may have refined stop/target on pendingEntry; prefer those if available
          const adjStop = (strat && strat.pos && Number.isFinite(+strat.pos.stop)) ? Number(strat.pos.stop) : Number(p.stop);
          const adjTarget = (strat && strat.pos && Number.isFinite(+strat.pos.target)) ? Number(strat.pos.target) : Number(p.target);
          // assign deterministic trade id at track start
          const nextId = `${runId}-${(tradeSeq + 1)}`;
          open = { entry: Number(fill.price), stop: adjStop, target: adjTarget, qty: Number(p.qty), ts: String(fill.ts||''), entryReason: p.reason || '', barsHeld: 0, tradeId: nextId, categoryTag: (typeof categoryKey==='string'?categoryKey:undefined), signalParams: p.signalParams || undefined };
          // log the fill
          try {
  ...
  // process all remaining candles (honor fast mode by limiting steps)
  try {
    const FAST_BARS = Number(process.env.BACKTEST_QUICK_BARS || 50);
    if (params && params.fast) {
      let steps = 0;
      while (replay && replay.running && steps < FAST_BARS) { replay.step(); steps++; }
      // if fast mode and we stopped early, mark replay as finished to trigger end-of-data behavior
      replay.running = false;
    } else {
      while (replay && replay.running) replay.step();
    }
  } catch(e) { while (replay && replay.running) replay.step(); }
  ...
  // Normalizer helper
    ...
    // Optional debug snapshots of normalization
    try {
      if (String(process.env.RESULT_DEBUG_SNAPSHOTS || '') === '1') {
        const dbgDir = path.resolve(__dirname, '..', 'jobs');
        try { fs.mkdirSync(dbgDir, { recursive: true }); } catch(_){ }
        const prePath = path.join(dbgDir, `job_${runId}.before-normalize.json`);
        const preFixed = path.join(dbgDir, 'debug_before_normalize.json');
        fs.writeFileSync(prePath, JSON.stringify(jobObj, null, 2), 'utf8');
        try { fs.writeFileSync(preFixed, JSON.stringify(jobObj, null, 2), 'utf8'); } catch(_){ }
      }
    } catch(_) {}
    try { jobObj = normalizeJob(jobObj); } catch(_) {}

    // Final normalization: ensure top-level required fields and types
    try {
      jobObj.error = jobObj.error === undefined ? null : jobObj.error;
      jobObj.summary = jobObj.summary || {};
      // canonical numeric summary fields
      jobObj.summary.totalPnL = Number(jobObj.summary.totalPnL || jobObj.summary.netPnl || 0);
      jobObj.summary.winRate = Number(jobObj.summary.winRate || 0);
      jobObj.summary.avgReturn = Number(jobObj.summary.avgReturn || jobObj.summary.expectancy || 0);
      jobObj.summary.tradesCount = Number(jobObj.summary.tradesCount || jobObj.totalTrades || jobObj.tradesCount || 0);
      jobObj.summary.maxDrawdown = Number(jobObj.summary.maxDrawdown || 0);

      // Normalize winRate to 0-100 and convert fractional 0-1 to percentage
      let wr = Number(jobObj.summary.winRate);
      if (!Number.isFinite(wr)) wr = 0;
      if (wr <= 1 && wr >= 0) wr = wr * 100;
      if (!Number.isFinite(wr)) wr = 0;
      wr = Math.max(0, Math.min(100, +wr.toFixed(2)));
      jobObj.summary.winRate = wr;

      // Top-level convenience fields per requested shape
  // If there were no trades, prefer null/absent accuracy to avoid misleading 0.00 in the UI
  jobObj.accuracy = (jobObj.summary.tradesCount && Number(jobObj.summary.tradesCount) > 0) ? wr : null;
  jobObj.winRate = (jobObj.summary.tradesCount && Number(jobObj.summary.tradesCount) > 0) ? wr : null;
      jobObj.expectancy = Number(jobObj.summary.avgReturn || 0);
      jobObj.totalPnL = Number(jobObj.summary.totalPnL || 0);
      jobObj.maxDrawdown = Number(jobObj.summary.maxDrawdown || 0);

      // ensure perSymbol exists and is object
      jobObj.perSymbol = jobObj.perSymbol || {};

      // Build top-level trades array (normalized) from allTrades captured earlier
      try {
        jobObj.trades = Array.isArray(allTrades) ? allTrades.map(t => {
          const out = {};
          out.symbol = String(t.symbol || '');
          out.entryTs = t.entryTime || t.entryTs || t.entry_time || null;
          out.entryPrice = (typeof t.entry === 'number') ? t.entry : (typeof t.entryPrice === 'number' ? t.entryPrice : null);
          out.exitTs = t.time || t.exitTime || t.exitTs || null;
          out.exitPrice = (typeof t.exit === 'number') ? t.exit : (typeof t.exitPrice === 'number' ? t.exitPrice : null);
      ...
      } catch (_) { jobObj.trades = jobObj.trades || []; }

      // Ensure status and timestamps exist
      jobObj.status = jobObj.status || 'done';
      jobObj.createdAt = jobObj.createdAt || (new Date()).toISOString();
      jobObj.completedAt = (jobObj.completedAt === undefined) ? (new Date()).toISOString() : jobObj.completedAt;
    } catch (_) { /* best-effort normalization; fall through */ }
    try {
      if (String(process.env.RESULT_DEBUG_SNAPSHOTS || '') === '1') {
        const dbgDir = path.resolve(__dirname, '..', 'jobs');
        const postPath = path.join(dbgDir, `job_${runId}.after-normalize.json`);
        const postFixed = path.join(dbgDir, 'debug_after_normalize.json');
        fs.writeFileSync(postPath, JSON.stringify(jobObj, null, 2), 'utf8');
        try { fs.writeFileSync(postFixed, JSON.stringify(jobObj, null, 2), 'utf8'); } catch(_){ }
      }
    } catch(_) {}
...
if (require.main === module) {
  ...
  else if (a==='--quiet-rejects'){ /* flag */ params_quiet_rejects = true; }
  ...
  const runParams = { symbols, from, to, interval, mode, strategyConfig, resumeRunId: resumeRunId || undefined, runId: runIdFlag || undefined };
  if (typeof params_quiet_rejects !== 'undefined') runParams.quietRejects = true;
  const out = await runBacktest(runParams, { onLog: (l)=>console.log(l) });
  ...
  // Emit aggregated rejection summaries when quietRejects enabled
  try {
    if (quietRejects) {
      for (const s of Object.keys(rejectCounters)) {
        const entry = rejectCounters[s];
        try { hooks && hooks.onLog && hooks.onLog(`REJECTED_SUMMARY ${runId} ${s} count=${entry.count} samples=${Array.from(entry.samples).slice(0,3).map(x => JSON.stringify(x)).join('|')}`); } catch(_){ }
      }
    }
  } catch(_){ }