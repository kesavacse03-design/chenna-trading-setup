...
class LiveEngine {
  ...
  try { runner.lastCandleTs = date ? new Date(String(date)).getTime() : Date.now(); } catch (_) { runner.lastCandleTs = Date.now(); }
    ...
    // Optionally skip signals on partial/incomplete bars
    try {
      if (String(process.env.NO_PARTIAL_BAR_SIGNALS||'') === '1') {
        const isPartial = !!(candle && (candle.partial === true || candle.isPartial === true || candle.is_final === false));
        if (isPartial) {
          // still advance indicators/history but do not emit entries
  ...
  try { this.audit.log('trade_exit', { symbol, side: 'LONG', reason: 'target', exit: exitPx, pnl, qty: openPos.qty }); } catch (_) {}
      ...
      // Swing expiry enforcement
      try {
        const maxDays = Number(process.env.SWING_MAX_HOLD_DAYS||0) || 0;
        if (maxDays > 0) {
          const holdingDays = (() => {
            try { const e = new Date(String(openPos.ts||'')); const x = new Date(String(c.date||'')); return Math.max(0, Math.round((x - e) / (1000*60*60*24))); } catch(_) { return 0; }
          })();
          if (holdingDays >= maxDays) {
            const exitPx = +c.close;
            const pnl = (exitPx - openPos.entry) * openPos.qty;
            runner.currentCapital += pnl;
            const trade = { symbol, time: String(c.date||''), entryTime: String(openPos.ts||''), entry: openPos.entry, exit: exitPx, exitReason: 'expiry', qty: openPos.qty, stop: openPos.stop, target: openPos.target, pnl: +pnl.toFixed(2), category:'BREAKOUT', direction:'LONG', holdingDays, reason: openPos.entryReason||'', trace:{ reason: 'expiry' } };
            this.allTrades.push(trade);
            runner.currentExposure -= Math.abs(openPos.entry * openPos.qty);
            runner.metrics.trades++;
            runner.lastActionTs = Date.now();
            if (this.onLog) this.onLog(`STRATEGY: exit ${symbol} @${exitPx.toFixed(2)} (expiry ${maxDays}d)`);
            try { this.audit.log('trade_exit', { symbol, side:'LONG', reason:'expiry', exit: exitPx, pnl, qty: openPos.qty }); } catch(_) {}
            this.openPositions.delete(symbol);
          }
        }
      } catch(_) {}
    ...
    if (p && !this.openPositions.has(symbol)) {
      // Enforce max concurrent trades cap
      try {
        const maxConc = Number(process.env.MAX_CONCURRENT_TRADES||0) || 0;
        if (maxConc > 0 && this.openPositions.size >= maxConc) {
          if (this.onLog) this.onLog(`Max concurrent trades reached (${this.openPositions.size}/${maxConc})`);
          try { this.audit.log('reject_entry', { symbol, reason:'max-concurrent', current: this.openPositions.size, cap: maxConc }); } catch(_) {}
          return;
        }
      } catch(_) {}
      ...
      let riskDistance = Math.max(0, entryPx - Number(p.stop || entryPx));
      // Apply slippage to risk distance if configured
      try { const slBps = Number(process.env.SLIPPAGE_BPS||0)||0; if (slBps>0) riskDistance += (entryPx * (slBps/10000)); } catch(_) {}
      ...
      // Cancel if spread too wide when spread info present
      try {
        const cancelBps = Number(process.env.CANCEL_IF_SPREAD_BPS||0)||0;
        const sp = Number(c.spreadBps || c.spread_bps || 0) || 0;
        if (cancelBps>0 && sp>0 && sp >= cancelBps) {
          if (this.onLog) this.onLog(`Spread too wide: ${sp}bps >= ${cancelBps}bps`);
          try { this.audit.log('reject_entry', { symbol, reason:'wide-spread', spreadBps: sp, capBps: cancelBps }); } catch(_) {}
          return;
        }
      } catch(_) {}

      // Risk-based sizing (bps of equity)
      try {
        const bps = Number(process.env.MAX_RISK_PER_TRADE_BPS||0)||0;
        if (bps>0) {
          const equity = Number(runner.currentCapital||0) || 0;
          const riskRs = (equity * (bps/10000));
          const qtyCap = Math.max(0, Math.floor(riskRs / Math.max(0.01, riskDistance)));
          if (qtyCap > 0 && Number(p.qty) > qtyCap) {
            if (this.onLog) this.onLog(`Sizing clipped by risk: qty ${p.qty} -> ${qtyCap} (riskRs=${riskRs.toFixed(2)} riskDist=${riskDistance.toFixed(2)})`);
            p.qty = qtyCap;
          } else if (qtyCap <= 0) {
            if (this.onLog) this.onLog('Risk sizing yielded 0 qty; rejecting signal');
            try { this.audit.log('reject_entry', { symbol, reason:'zero-qty-risk', riskRs, riskDistance }); } catch(_) {}
            return;
          }
        }
      } catch(_) {}
...
class LiveRunner {
  ...
  // Immediate in-process load: if an override token file exists and decrypts, set
  // UPSTOX_ACCESS_TOKEN in process.env so the running process uses it and avoids
  // attempting to read/write the primary (which may be read-only).
  try {
    const fs = require('fs');
    const tm = this.tokenManager;
    const keyName = tm.kEnv || 'TOKENS_ENCRYPTION_KEY';
    const key = process.env[keyName];
    const overridePath = tm.overrideFile;
    if (key && overridePath && fs.existsSync(overridePath)) {
      // Use decrypt helper directly to avoid TokenManager fallback logic
      const { decrypt } = require('./tokenManager.cjs');
      try {
        const b64 = fs.readFileSync(overridePath, 'utf8').trim();
        const data = JSON.parse(decrypt(b64, key));
        if (data && data.access_token) {
          process.env.UPSTOX_ACCESS_TOKEN = String(data.access_token);
          try { this.audit.log('token_loaded_inproc', { source: 'override', length: String(data.access_token).length }); } catch (_) {}
          console.log('[INPROC] Loaded UPSTOX_ACCESS_TOKEN from override file');
        }
      } catch (e) {
        console.error('[INPROC_TOKEN_DECRYPT_ERR]', e && (e.stack || e.message || String(e)));
      }
    }
  } catch (e) {
    console.error('[INPROC_TOKEN_LOAD_ERR]', e && (e.stack || e.message || String(e)));
  }
  try {
      ...
      if (req.url === '/admin/store-token' && req.method === 'POST') {
        (async () => {
          ...
          try {
            ...
            // Also set in-memory env so the running process picks up the token immediately
            try {
              process.env.UPSTOX_ACCESS_TOKEN = String(accessToken);
              if (refreshToken) process.env.UPSTOX_REFRESH_TOKEN = String(refreshToken);
            } catch (_) {}
            try { this.audit.log('token_saved', { by:'admin', length: String(accessToken).length }); } catch(_) {}
            console.log('[ADMIN] Stored token and set process.env.UPSTOX_ACCESS_TOKEN (len=', String(accessToken).length, ')');
            setJson();
            res.end(JSON.stringify({ ok:true }));
          } catch (e) {
            // Ensure we surface full error stacks to pod logs for debugging token save failures
            try { console.error('[STORE_TOKEN_ERR]', e && (e.stack || e.message || String(e))); } catch(_) {}
  ...
  try { this.audit.log('health_ping', { wsClients: this.wss ? this.wss.clients.size : 0 }); } catch(_) {}
      ...
      // Helper functions for cts_stocks.json
      const readStocks = () => {
        try {
          const fs = require('fs');
          const path = require('path');
          const file = path.resolve(process.cwd(), 'cts_stocks.json');
          if (!fs.existsSync(file)) return { list: [], file };
          const raw = fs.readFileSync(file, 'utf8');
          const list = raw ? JSON.parse(raw) : [];
          return { list: Array.isArray(list) ? list : [], file };
        } catch (_) { return { list: [], file: null }; }
      };
      const writeStocks = (file, list) => {
        const fs = require('fs');
        try { fs.writeFileSync(file, JSON.stringify(list, null, 2), 'utf8'); } catch (_) {}
      };

      // Serve the cts_stocks mirror if present so frontend can read existing stocks
      if (req.url === '/api/stocks' && req.method === 'GET') {
        (async () => {
          try {
      ...
      // Create a stock record (dedupe by symbol/date/category)
      if (req.url === '/api/stocks' && req.method === 'POST') {
        (async () => {
          try {
            const body = await readBody();
            const symbol = String((body && (body.symbol || body.stockName)) || '').toUpperCase();
            const date = String((body && body.date) || '').slice(0, 10);
            const category = String((body && (body.categoryKey || body.category || body.categoryRaw)) || '');
            if (!symbol || !date || !category) { res.writeHead(400, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ ok:false, error:'missing fields' })); return; }
            const { list, file } = readStocks();
            // broad duplicate check: match symbol+date and any of category fields
            const dup = list.find((s) => {
              try {
                const catKey = String(s.categoryKey || '').toUpperCase();
                const cat = String(s.category || '').toUpperCase();
                const catRaw = String(s.categoryRaw || '').toUpperCase();
                const want = category.toUpperCase();
                return (String(s.stockName||s.symbol).toUpperCase() === symbol) && (String(s.date||'').slice(0,10) === date) && (catKey === want || cat === want || catRaw === want);
              } catch(_) { return false; }
            });
            if (dup) { res.writeHead(409, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ ok:false, error:'DUPLICATE' })); return; }
            const now = new Date().toISOString();
            const id = Date.now();
            const item = { id, stockName: symbol, date, category, categoryKey: category, addedDate: now };
            list.push(item);
            if (file) writeStocks(file, list);
            setJson();
      ...
      // Delete by id
      if (req.url && req.method === 'DELETE' && req.url.startsWith('/api/stocks/')) {
        try {
          const idStr = req.url.split('/').pop();
          const id = Number(idStr);
          const { list, file } = readStocks();
          const idx = list.findIndex((s) => Number(s && s.id) === id);
          if (idx === -1) { res.writeHead(404, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ ok:false, error:'NOT_FOUND' })); return; }
          list.splice(idx, 1);
          if (file) writeStocks(file, list);
          setJson();
