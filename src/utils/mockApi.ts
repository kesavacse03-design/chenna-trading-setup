...
// --- HELPERS ---
// tradeIdCounter and getToday were previously declared but unused; removed to satisfy compiler.
...
// --- Stocks API (preview-mode localStorage-backed) ---
export function getStocks(): any[] {
    // stored as array of { id?, stockName, date, category, price?, exchange?, __pendingSync?, __pendingDelete? }
    try {
...
export function getPrices(symbols: string[]) {
    // return deterministic mock prices based on symbol hash
    const out: Record<string, { price: number; timestamp: string }> = {};
    const now = new Date().toISOString();
    for (const s of symbols) {
        // simple deterministic pseudo-price
        let h = 0; for (let i=0;i<s.length;i++) h = ((h<<5)-h) + s.charCodeAt(i);
        const price = Math.abs(h % 1000) + ((h % 100) / 100);
        out[s] = { price: Math.round(price * 100) / 100, timestamp: now };
    }
    return out;
}

export function createStock(payload: { symbol: string; date: string; category: string; categoryRaw?: string; categoryKey?: string }) {
    const list = getStocks();
    // normalize incoming category to ensure canonical keys are stored
    try {
        const resolved = normalizeAndMapCategory(payload.category || payload.categoryRaw || payload.categoryKey || '');
        payload.categoryRaw = resolved.categoryRaw || payload.categoryRaw;
        payload.categoryKey = resolved.categoryKey || payload.categoryKey;
        // store canonical category string for consistency
        payload.category = payload.categoryKey || payload.categoryRaw || payload.category;
    } catch (_) {}
    // dedupe by (symbol,date,category)
    const exists = list.find((s: any) => s.stockName === payload.symbol && s.date === payload.date && s.category === payload.category && !s.__pendingDelete);
    if (exists && exists.id) {
    ...
    const now = new Date().toISOString();
    const item: any = { id: Date.now() + Math.floor(Math.random() * 1000), stockName: payload.symbol, date: payload.date, category: payload.category, addedDate: now };
    if (payload.categoryRaw) item.categoryRaw = payload.categoryRaw;
    if (payload.categoryKey) item.categoryKey = payload.categoryKey;
    list.push(item);
