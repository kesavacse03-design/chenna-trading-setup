...
(async ()=>{
  try{
    ...
    for (let i=0;i<60;i++){
      ...
      const logs = st.logs||[];
      console.log('tick',i,'status',st.status,'logs count',logs.length);
      const found = logs.find(l=>l.includes('RESULT: job JSON will be written to'))||logs.find(l=>l.includes('RESULT: wrote job JSON'))||logs.find(l=>l.includes('RESULT: failed to write job JSON'));
      if (found) console.log('FOUND:',found);
      if (st.status==='done') { console.log('DONE logs:\n', logs.join('\n')); break; }
