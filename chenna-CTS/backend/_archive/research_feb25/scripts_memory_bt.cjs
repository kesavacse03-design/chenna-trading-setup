...
async function run() {
    ...
    console.log(`Loaded ${Object.keys(dataMap).length} symbols. Running SHORT_TERM_SWING_BO_DOWN...`);
    ...
    for (const symbol in dataMap) {
        ...
        for (let i = 50; i < daily.length - 1; i++) {
            ...
            // Check signal
            // checkSignal(stock, dailyCandles, weeklyCandles, niftyCandles)
            // We'll pass empty arrays for weekly/nifty, relies on daily fallback
            const signal = await swingBoDown.checkSignal(symbol, pastDaily, [], []);
    ...
    console.log(`Total Trades Executed: ${atrTrailTrades.length}`);
    ...
    console.log(`\nOverall: ${atrTrailTrades.length} trades | WR: ${winRate.toFixed(2)}% | P&L: ${avgPnl.toFixed(2)}%`);
    ...
    console.log("\nSample Trades:");
    for (const t of atrTrailTrades.slice(0, 10)) {
