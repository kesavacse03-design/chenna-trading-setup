...
const CATEGORY_KEY = 'LONG_TERM_SWING_BO_DOWN';
const OUTPUT_FILE = path.join(__dirname, '../results/lt_down_data.json');
    ...
    try {
        ...
        for (let i = 0; i < allStocks.length; i += BATCH_SIZE) {
            ...
            await Promise.all(batch.map(async (item) => {
                ...
                // Calculate ranges
                const startDaily = new Date(addedDate); startDaily.setDate(startDaily.getDate() - 30);
                const endDaily = new Date(addedDate); endDaily.setDate(endDaily.getDate() + 15);

                const startWeekly = new Date(addedDate); startWeekly.setDate(startWeekly.getDate() - 180);
        ...
        if (e.message === 'RATE_LIMIT_HIT') {
            ...
            console.error('Please re-login to Upstox to refresh the limit.');
        } else {
            console.error('\n❌ Error:', e);
        }
    } finally {
