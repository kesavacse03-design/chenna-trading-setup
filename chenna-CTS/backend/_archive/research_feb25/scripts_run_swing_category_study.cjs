...
const CATEGORY_KEY = process.argv[2] || 'SHORT_TERM_SWING_BO_DOWN';
const filePrefix = CATEGORY_KEY.toLowerCase();

const PROFILES_PATH = path.join(OUT_DIR, `${filePrefix}_profiles.json`);
const SUMMARY_CSV_PATH = path.join(OUT_DIR, `${filePrefix}_summary.csv`);
...
async function main() {
    console.log(`[Studying ${CATEGORY_KEY} Multiframe Profiles]`);
    ...
    if (!category) {
        console.error(`Category ${CATEGORY_KEY} not found in DB!`);
        process.exit(1);
    }
