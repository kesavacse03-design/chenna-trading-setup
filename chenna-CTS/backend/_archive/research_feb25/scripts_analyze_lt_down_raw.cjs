 ...
 * Segments: By Pre-Trend (Crash vs Fresh).
 */
...
const REPORT_FILE = path.join(__dirname, '..', 'results', 'lt_swing_bo_down_raw_analysis.md');
const CSV_FILE = path.join(__dirname, '..', 'results', 'lt_swing_bo_down_raw_metrics.csv');

const round2 = (v) => Math.round(v * 100) / 100;

async function main() {
    console.log('══════════════════════════════════════════════════════════════════════');
    console.log('  RAW OUTCOME ANALYSIS (No Strategy Filters)');
    console.log('══════════════════════════════════════════════════════════════════════');
    ...
    for (const symbol of symbols) {
        ...
        // If 20 days not available, use max available
...
function generateReport(metrics) {
    let md = '# RAW OUTCOME ANALYSIS: LT_SWING_BO_DOWN\n\n';
    ...
    for (const w of [5, 10, 20]) {
        ...
        let edge = 'NEUTRAL';
        if (ratio > 1.5) edge = 'LONG 🟢';
        else if (ratio < 0.67) edge = 'SHORT 🔴';

        md += `| **${w} Days** | +${avgUp}% | ${avgDown}% | **${ratio}x** | ${edge} |\n`;
    }
    md += '\n';

    // B. Segmentation by Pre-Trend
    md += '## 2. Segmentation by Pre-Trend (20d)\n\n';

    const segments = [
        { label: 'Deep Crash (<-15%)', filter: m => m.preTrendPct < -15 },
        { label: 'Moderate (-15% to -5%)', filter: m => m.preTrendPct >= -15 && m.preTrendPct < -5 },
        { label: 'Fresh/Flat (>-5%)', filter: m => m.preTrendPct >= -5 }
    ];
