 ...
 * Windows: 5-day, 10-day, 20-day.
 * Segments: By Pre-Trend (Rally Strength).
 */
...
// Config
const DATA_FILE = path.join(__dirname, '..', 'results', 'lt_up_data.json');
const REPORT_FILE = path.join(__dirname, '..', 'results', 'lt_swing_bo_up_raw_analysis.md');
const CSV_FILE = path.join(__dirname, '..', 'results', 'lt_swing_bo_up_raw_metrics.csv');

const round2 = (v) => Math.round(v * 100) / 100;

async function main() {
    console.log('══════════════════════════════════════════════════════════════════════');
    console.log('  RAW OUTCOME ANALYSIS (No Strategy Filters) - LT_SWING_BO_UP');
    console.log('══════════════════════════════════════════════════════════════════════');
    ...
    for (const symbol of symbols) {
        ...
        for (const w of windows) {
            ...
            res[`close${w}d`] = round2((future[future.length - 1].close - entryPrice) / entryPrice * 100);
        }
...
function generateReport(metrics) {
    let md = '# RAW OUTCOME ANALYSIS: LT_SWING_BO_UP\n\n';
    ...
    for (const w of [5, 10, 20]) {
        ...
        let edge = 'NEUTRAL';
        if (ratio > 1.25) edge = 'LONG 🟢'; // Lower threshold for Trend Following?
    ...
    // B. Segmentation by Pre-Trend (Rally Strength)
    md += '## 2. Segmentation by Pre-Trend (20d)\n\n';

    // For UP, pre-trend is usually positive (breakout).
    // Categories: Strong Rally (>15%), Moderate (5-15%), Flat/Weak (<5%).
    const segments = [
        { label: 'Strong Rally (>15%)', filter: m => m.preTrendPct > 15 },
        { label: 'Moderate Rally (5% to 15%)', filter: m => m.preTrendPct >= 5 && m.preTrendPct <= 15 },
        { label: 'Flat/Weak (<5%)', filter: m => m.preTrendPct < 5 }
    ];
