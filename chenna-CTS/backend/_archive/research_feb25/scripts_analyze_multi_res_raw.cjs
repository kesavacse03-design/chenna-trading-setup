 ...
 * Category: MULTI_RESISTANCE_BO (Breakout)
 */
...
// Config
const DATA_FILE = path.join(__dirname, '..', 'results', 'multi_res_data.json');
const REPORT_FILE = path.join(__dirname, '..', 'results', 'multi_res_bo_raw_analysis.md');
const CSV_FILE = path.join(__dirname, '..', 'results', 'multi_res_bo_raw_metrics.csv');

const round2 = (v) => Math.round(v * 100) / 100;

async function main() {
    console.log('══════════════════════════════════════════════════════════════════════');
    console.log('  RAW OUTCOME ANALYSIS - MULTI_RESISTANCE_BO');
    console.log('══════════════════════════════════════════════════════════════════════');
...
function generateReport(metrics) {
    let md = '# RAW OUTCOME ANALYSIS: MULTI_RESISTANCE_BO\n\n';
    ...
    // Categories: Strong Rally (>15%), Moderate (5-15%), Flat/Base (<5%), Drop (<-5%).
    const segments = [
        { label: 'Strong Rally (>15%)', filter: m => m.preTrendPct > 15 },
        { label: 'Moderate Rally (5% to 15%)', filter: m => m.preTrendPct >= 5 && m.preTrendPct <= 15 },
        { label: 'Base/Flat (-5% to 5%)', filter: m => m.preTrendPct >= -5 && m.preTrendPct < 5 },
        { label: 'Reversal from Drop (<-5%)', filter: m => m.preTrendPct < -5 }
    ];
