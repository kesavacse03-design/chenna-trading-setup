    ...
    for (const cat of INITIAL_CATEGORIES) {
        await prisma.category.upsert({
            ...
            update: {
                ...
                // Do not overwrite enabled status if it exists, but we can't conditinally update here easily without raw query
                // For now, let's just update name/desc. 
                // Creating new ones will get defaults.
            },
            create: {
                key: cat.key,
                name: cat.name,
