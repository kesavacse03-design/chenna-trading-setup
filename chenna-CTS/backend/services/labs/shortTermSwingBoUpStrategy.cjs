/**
 * SHORT_TERM_SWING_BO_UP — Labs Backtest Wrapper
 * 
 * This file is a THIN WRAPPER around the source-of-truth strategy:
 *   strategies/swingBoUpLongStrategy.cjs
 * 
 * It provides the backtest() function expected by backtestRoutes.cjs,
 * but delegates ALL signal logic to the real strategy.
 * 
 * Direction: LONG (buy breakout)
 * Tier 1: RSI < 70 (Fresh Breakout) → TRADE
 * Tier 2: RSI >= 70 (Extended Momentum) → SKIP
 * Stops: ATR-based via calculateIntelligentStop()
 */

const { PrismaClient } = require('@prisma/client');
const prisma = new PrismaClient();
const realStrategy = require('../../strategies/swingBoUpLongStrategy.cjs');
const { calculateTrailingStop } = require('../stopLossCalculator.cjs');

const CONFIG = {
    name: 'SHORT_TERM_SWING_BO_UP',
    displayName: 'Short-Term Swing BO Up (Momentum Breakout)',
    timeframe: 'daily',
...
async function backtest(startDate, endDate) {
    ...
    console.log(`[ST_SWING_BO_UP] Backtesting ${stocks.length} unique stocks from ${startDate} to ${endDate}`);
    ...
    console.log(`[ST_SWING_BO_UP] Done: ${stats.totalTrades} trades, WR: ${stats.winRate.toFixed(1)}%, P&L: ${stats.totalPnl.toFixed(2)}%`);
