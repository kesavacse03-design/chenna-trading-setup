/**
 * LONG_TERM_SWING_BO_DOWN — Labs Backtest Wrapper
 * 
 * This file is a THIN WRAPPER around the source-of-truth strategy:
 *   strategies/ltSwingBoDownLongStrategy.cjs
 * 
...
const realStrategy = require('../../strategies/ltSwingBoDownLongStrategy.cjs');
const { calculateTrailingStop } = require('../stopLossCalculator.cjs');

const CONFIG = {
    name: 'LONG_TERM_SWING_BO_DOWN',
    displayName: 'Long-Term Swing BO Down (Mean Reversion)',
    timeframe: 'daily',
    category: 'LONG_TERM_SWING_BO_DOWN',
    direction: 'LONG',
...
async function getCategoryStocks() {
    const cat = await prisma.category.findUnique({
        where: { key: 'LONG_TERM_SWING_BO_DOWN' },
...
async function backtest(startDate, endDate) {
    ...
    console.log(`[LT_SWING_BO_DOWN] Backtesting ${stocks.length} unique stocks from ${startDate} to ${endDate}`);
    ...
    for (const s of stocks) {
        ...
        if (candles.length < 40) continue; // Needs 30 min per strat rules
    ...
    console.log(`[LT_SWING_BO_DOWN] Done: ${stats.totalTrades} trades, WR: ${stats.winRate.toFixed(1)}%, P&L: ${stats.totalPnl.toFixed(2)}%`);
