/**
 * MULTI_SUPPORT_BO — Labs Backtest Wrapper
 * 
 * This file is a THIN WRAPPER around the source-of-truth strategy:
 *   strategies/multiSupportBoLongStrategy.cjs
 * 
 * It provides the backtest() function expected by backtestRoutes.cjs,
 * but delegates ALL signal logic to the real strategy.
 * 
 * Direction: LONG (mean reversion / bear trap)
 * Filter: NONE (All pre-trends work for this setup)
 * Stops: ATR-based via calculateIntelligentStop()
 */

const { PrismaClient } = require('@prisma/client');
const prisma = new PrismaClient();
const realStrategy = require('../../strategies/multiSupportBoLongStrategy.cjs');
const { calculateTrailingStop } = require('../stopLossCalculator.cjs');

const CONFIG = {
    name: 'MULTI_SUPPORT_BO',
    displayName: 'Multi-Day Support Breakout (Bear Trap)',
    timeframe: 'daily',
    category: 'MULTI_SUPPORT_BO',
    direction: 'LONG',
    maxHoldDays: 20, // 20 days per 249-stock analysis: +0.53% avg at 20d
    trailPct: 0.04,
    trailActivation: 0.02,
    breakevenPct: 0.03
};
...
async function backtest(startDate, endDate) {
    ...
    console.log(`[MULTI_SUPPORT_BO] Backtesting ${stocks.length} unique stocks from ${startDate} to ${endDate}`);
    ...
    console.log(`[MULTI_SUPPORT_BO] Done: ${stats.totalTrades} trades, WR: ${stats.winRate.toFixed(1)}%, P&L: ${stats.totalPnl.toFixed(2)}%`);
