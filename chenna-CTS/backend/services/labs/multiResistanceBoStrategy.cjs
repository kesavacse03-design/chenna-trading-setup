/**
 * MULTI_RESISTANCE_BO — Labs Backtest Wrapper
 * 
 * This file is a THIN WRAPPER around the source-of-truth strategy:
 *   strategies/multiResistanceBoStrategy.cjs
 * 
 * It provides the backtest() function expected by backtestRoutes.cjs,
 * but delegates ALL signal logic to the real strategy.
 * 
 * Direction: LONG (flat base breakout)
 * Filter: Pre-trend must be -5% to +5%
 * Stops: ATR-based via calculateIntelligentStop()
 */

const { PrismaClient } = require('@prisma/client');
const prisma = new PrismaClient();
const realStrategy = require('../../strategies/multiResistanceBoStrategy.cjs');
const { calculateTrailingStop } = require('../stopLossCalculator.cjs');

const CONFIG = {
    name: 'MULTI_RESISTANCE_BO',
    displayName: 'Multi-Day Resistance Breakout (Flat Base)',
    timeframe: 'daily',
    category: 'MULTI_RESISTANCE_BO',
    direction: 'LONG',
    maxHoldDays: 20, // 20 days per raw analysis
    trailPct: 0.05,
...
async function backtest(startDate, endDate) {
    ...
    console.log(`[MULTI_RESISTANCE_BO] Backtesting ${stocks.length} unique stocks from ${startDate} to ${endDate}`);
    ...
    console.log(`[MULTI_RESISTANCE_BO] Done: ${stats.totalTrades} trades, WR: ${stats.winRate.toFixed(1)}%, P&L: ${stats.totalPnl.toFixed(2)}%`);
