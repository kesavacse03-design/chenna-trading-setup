...
class TimeTravelBacktestEngine {
    ...
    async testAtDate(logic, stock, allCandles, dateIndex) {
        ...
        // STEP 4: Check institutional traps BEFORE entering
        const trapScan = this.trapDetector.scanAllTraps(availableCandles, context);

        // Skip if traps detected (this is our edge!)
        if (trapScan.recommendation === "AVOID") {
            return {
                ...
                exit: null,
                skipped: true,
                reason: `Trap detected: ${trapScan.traps[0]?.type}`,
                trapCount: trapScan.trapsDetected
            };
        }
        ...
        return {
            ...
            trapAvoidance: trapScan.trapsDetected === 0
        };
    }
