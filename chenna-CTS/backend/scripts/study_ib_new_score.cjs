...
async function evaluateNewScore() {
    ...
    for (const date of allTradingDates) {
        for (const stock of universeStocks) {
            ...
            if (score >= 95) { if (isWin) t1.w++; else t1.l++; }
            else if (score >= 65) { if (isWin) t2.w++; else t2.l++; }
    ...
    console.log(`TIER 1 (95-100 pts): ${fmt(t1.w, t1.l)}`);
    console.log(`TIER 2 (65-94 pts):  ${fmt(t2.w, t2.l)}`);
    console.log(`TIER 3 (<65 pts):    ${fmt(t3.w, t3.l)}`);
