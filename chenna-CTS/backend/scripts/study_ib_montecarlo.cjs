...
async function runMonteCarloSelection() {
    ...
    // Quick load all stocks to get sectors
    const allStocks = await prisma.stock.findMany();
    const sectorMap = {};
    allStocks.forEach(s => sectorMap[s.symbol] = s.sector);

    console.log(`[Engine] Reconstructing Daily Real-Time Signal Arrays...`);
    ...
    for (const date of validDates) {
        ...
        for (const stock of stocks) {
            ...
            // Optimization logic: Dual category edge is dead, but Price Range 1000-2000 has high WR (58.3%).
            let priceOptBonus = 0;
            if (boPrice >= 1000 && boPrice <= 2000) priceOptBonus = 10;
            else if (boPrice > 5000) priceOptBonus = -10;

            dailySignals.push({
                sym, bIdx, sType, boPrice, result, confScore,
                optScore: confScore + priceOptBonus
            });
        }

        // We only care about days where we have to choose (more than 3 valid breakouts generated)
        if (dailySignals.length > 3) {
            testDays.push(dailySignals);
        }
    }

    console.log(`[Engine] Found ${testDays.length} days where >3 active signals were generated simultaneously.\n`);

    // --- MONTE CARLO SIMULATION (Risking exactly flat 1.0R per trade) ---
    // A WIN = +1.0R, A LOSS = -1.0R

    const performance = {
        Strategy_A: { name: "Top 3 by Confidence Score", pnlR: 0, wins: 0, total: 0 },
        Strategy_B: { name: "First 3 Chronologically (Action Bias)", pnlR: 0, wins: 0, total: 0 },
        Strategy_C: { name: "3 Random Signals (Monkey)", pnlR: 0, wins: 0, total: 0 },
        Strategy_D: { name: "Optimized 3 (Score + Optimal Price Band)", pnlR: 0, wins: 0, total: 0 }
    };

    // Evaluate deterministic strategies (A, B, D)
    for (const day of testDays) {
        ...
        // B. First 3 Chrono (assuming bIdx represents time)
    ...
    // Evaluate Monte Carlo Random Strategy C (1000 iter avg)
    ...
    console.log(`=== MAXIMUM 3 SELECTION LOGIC SIMULATION ===\n`);
    for (const key in performance) {
        const d = performance[key];
        const wr = (d.wins / d.total) * 100;
        console.log(`[${key}] ${d.name}: \n   -> WR: ${wr.toFixed(1)}% | Generated PnL: ${(d.pnlR).toFixed(1)} R\n`);
    }

}

runMonteCarloSelection().then(() => prisma.$disconnect());
