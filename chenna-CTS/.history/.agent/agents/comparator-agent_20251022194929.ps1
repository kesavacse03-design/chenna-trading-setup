...
Write-Host "📊 Loaded $($history.Count) valid entries from shadow files"

# Group by symbol
$grouped = $history | Group-Object { $_.symbol }
$summary = @()
...
foreach ($g in $grouped) {
    ...
    $total = $entries.Count
    $pnlSum = ($entries | Measure-Object -Property pnl -Sum).Sum
    $mdd = ($entries | Measure-Object -Property pnl -Minimum).Minimum
